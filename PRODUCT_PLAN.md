...
> 분석 가능한 수시간짜리 방송 원본과 선택적 자막·채팅 기록을 AI가 먼저 끝까지 훑고, 사람이 짧은 추천 후보만 재생해 30~60초 클립과 긴 하이라이트를 확정하는 로컬 우선 편집 보조 웹앱이다.
...
|---|---:|---:|---:|---:|---:|
| 내 컴퓨터의 영상 파일 | 브라우저/코덱 지원 시 가능 | 가능 | 가능 | 영상·음성 로컬 AI 가능 | 조건 충족 시 가능 |
...
타임라인 손잡이는 제공하되 드래그만 강요하지 않는다. 숫자 입력, 버튼, 방향키를 함께 지원한다. 프레임 단위 편집은 고급 기능이며 기본은 시:분:초와 1초 조절이다.
...
현재 `0.3.1` fast pass는 이 원칙을 다음과 같이 실제 구현한다.
