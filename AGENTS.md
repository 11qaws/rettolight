...
- GitHub Pages 정적 앱에서 원본 선택 → 분석 → 후보 검토 → 메타데이터 출력까지 서버 없이 완주해야 한다.
- 회원가입, 사용자 계정, 팀 공유, 동시 공동 편집, 원격 프로젝트 DB, 기기간 자동 동기화, 공용 백엔드, 클라우드 AI, 게시 대행은 범위 밖이다.
...
- 선택한 로컬 영상·음성·채팅을 외부 분석 서비스로 보내지 않는다.
- AI 모델과 앱 정적 파일 다운로드, 사용자가 연 플랫폼 플레이어 통신만 제품에 설명된 네트워크 예외다.
- 비밀 API key, CHZZK Client Secret, access/refresh token을 Pages 번들, URL, localStorage, 프로젝트 JSON, 로그, fixture, 문서에 넣지 않는다.
