...
- 이 제품은 **1인용 로컬 우선 AI 편집 어시스턴트**다.
- 핵심 가치는 수시간 원본을 사람이 전부 보는 대신 AI가 먼저 하이라이트 후보를 찾고, 사람이 짧은 후보만 검토하게 하는 것이다.
- GitHub Pages 정적 앱에서 원본 선택 → 로컬 빠른 분석 → 후보 검토 → 메타데이터 출력까지 서버 없이 완주해야 한다. 선택형 Gemini 정밀 분석이 실패하거나 설정되지 않아도 이 핵심 경로는 유지한다.
- 회원가입, 사용자 계정, 팀 공유, 동시 공동 편집, 원격 프로젝트 DB, 기기간 자동 동기화, 공용 백엔드, 게시 대행은 범위 밖이다. 클라우드 AI는 사용자가 직접 자기 Gemini API 키를 넣고 전송 범위를 확인한 뒤, 로컬 빠른 분석이 고른 짧은 후보 오디오만 보내는 선택 기능으로 한정한다.
- 공용 지침의 소규모 공유 서비스 기본값은 사용자 결정에 따른 명시적 프로젝트 예외다. 단, 개인용 앱의 복구·배포·백업·롤백·진단 품질은 생략하지 않는다.
- CHZZK 공식 라이브 채팅 수집이 필요하면 사용자 컴퓨터에 설치하는 선택형 로컬 동반 수집기로만 설계한다. Pages 핵심 경로는 사용자가 가진 JSONL·JSON·CSV 채팅 기록 가져오기다.

## 3. 데이터·보안 경계

- 기본 경로에서는 선택한 로컬 영상·음성·채팅을 외부 분석 서비스로 보내지 않는다. 선택형 Gemini 정밀 분석에서는 명시적 동의 뒤 최대 12개의 30~60초 후보 오디오만 전송하며, 원본 전체·영상 프레임·파일명·채팅은 전송하지 않는다.
- AI 모델과 앱 정적 파일 다운로드, 사용자가 연 플랫폼 플레이어 통신, 사용자가 직접 시작한 후보 오디오 Gemini 요청만 제품에 설명된 네트워크 예외다.
- 비밀 API key, CHZZK Client Secret, access/refresh token을 Pages 번들, URL, localStorage, IndexedDB, 프로젝트 JSON, 로그, fixture, 문서에 넣지 않는다. Gemini 키는 사용자가 현재 탭에 직접 입력하고 실행 중 메모리에만 두며 오류 원문에도 복사하지 않는다.
