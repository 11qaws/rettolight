...
- 공용 키를 쓰는 클라우드 AI, 서버 FFmpeg, 게시 대행, 공개 갤러리
- 원격 텔레메트리와 사용자 행동 추적
...
|---|---|---|---|
| GitHub Pages 앱 | 공개 정적 사이트 | UI, Source Adapter, 분석 조정, 검토, 내보내기, 선택형 Gemini 후보 요청 | 공용 비밀값, 사용자 DB, 영상 프록시 |
...
- 기본 로컬 빠른 분석과 반응 종류 분석에서는 선택한 영상·오디오·채팅을 외부로 전송하지 않는다. 사용자가 Gemini 키와 전송 범위를 확인한 선택형 정밀 분석만 후보 오디오를 Google API로 보낸다.
...
아래 항목은 이전 배포의 재현·롤백 기록이다. 현재 `0.3.7` 제품 경로에는 로컬 Whisper가 번들되지 않으며, 실제 운영 경계는 2.6절의 사용자 시작형 Gemini 후보 분석이다.
...
- 배포 확인: `/rettolight/`의 main JS·CSS·Worker·WASM이 200이고 정적 번들에 공용 키가 없는지 확인한다. Gemini 요청은 명시적 키·동의 뒤에만 후보 오디오로 구성되는지 mock으로 검사한다. 키보드 summary/cue focus, 320~390px 폭, 밝은 테마의 chat/visual badge 대비와 console warning/error도 확인한다.

### 2.6 `0.3.7` Gemini 후보 정밀 분석 운영 경계

- 핵심 폴백: Gemini를 설정하지 않거나 API가 실패해도 GitHub Pages의 로컬 fast pass, 선택형 채팅 결합, 후보 재생·검토, 반응 종류 로컬 AI, 내보내기는 완주한다.
- 처리 범위: 최대 12시간 원본 전체를 전송하지 않는다. fast pass가 고른 최대 12개의 30~60초 후보 오디오만 한 후보씩 16kHz mono PCM16 WAV로 만들며 실행 전에 후보 수와 합계 시간을 표시한다.
- 원격 경계: `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash:generateContent`에 `audio/wav` inline data와 고정 prompt/schema만 보낸다. 원본 파일명·영상 프레임·채팅·후보 점수·사람 검토 상태는 body, URL, header, 로그에 넣지 않는다.
- 키 경계: 공용 키를 repository, GitHub Actions secret을 통한 정적 bundle, URL, Local Storage, IndexedDB, Cache, OPFS, 프로젝트 backup, export, fixture, 로그에 넣지 않는다. 사용자 소유 키는 password input에서 현재 탭 메모리와 실행 Worker로만 전달하고 worker 종료·탭 종료 때 참조를 잃는다. Google 공식 권고상 서버 프록시보다 노출 위험이 크다는 안내와 Gemini 전용 키·할당량 제한 권장을 실행 전에 표시한다.
- Google 데이터 경계: 모든 `generateContent` 요청은 `store: false`로 고정하되 이를 즉시 삭제 보장으로 표현하지 않는다. 무료 tier는 제품 개선에 사용될 수 있고 유료 tier는 제품 개선에 사용되지 않으며, 악용 방지 데이터는 최대 55일 보관되고 조건에 따라 권한 있는 사람 검토가 있을 수 있음을 실행 전에 표시한다.
- 요청 크기: 60초 16kHz mono PCM16 WAV는 Base64 포함 약 2.6MB이며, 2026-07 공식 오디오 인라인 총 요청 20MB 한도보다 충분히 작다. 앱은 후보당 60초·Base64 8MB의 더 좁은 자체 안전 경계를 두고 한 건씩만 전송하며, Files API를 사용하지 않아 원격 파일 lifecycle을 만들지 않는다.
- 결과 검증: model JSON은 후보 상대 시간과 닫힌 문자열 필드만 허용한다. App으로 넘기기 전에 exact keys, 타입, 배열 수, 시간 정방향·후보 범위, NFKC·제어문자·길이 제한을 검증한다. candidate ID·절대 원본 범위는 로컬 snapshot에서 주입한다.
...
- 비용·오류: 자동 재시도하지 않는다. 인증/키, 할당량 429, 네트워크·5xx, 구조 오류를 서로 다른 redacted code로 안내하고 Google 원문 오류·키를 UI나 진단에 복사하지 않는다. 실패한 후보나 run은 기존 후보와 이전의 더 좋은 세션 단서를 지우지 않는다.
...
- 배포 확인: Pages origin의 CORS preflight가 `POST`, `content-type`, `x-goog-api-key`를 허용하는지 확인하되 실제 키를 CI·로그에 넣지 않는다. mock fetch로 요청·응답·오류·취소를 검사하고, 실키 smoke는 사용자가 자신의 브라우저에서 명시적으로 수행할 때만 기록한다.
