...
- 전체 맥락용 전사는 별도 예산 단계다. 완전한 외부 자막이 있으면 이를 우선 사용하고, 없으면 `qwen3-asr-flash-filetrans`의 `$0.000035/초` 기준 최대 `$0.42` 범위에서 모든 10분 블록을 고르게 표본화하고 최대 12개 사건 주변 2분을 포함한다. 현재 Worker transport가 활성화되어 있으며, credential·요청 검증·예산 guard 중 하나라도 통과하지 못하면 upstream 호출 전에 fail-closed 한다.
...
- 요청 경계: Pages Worker는 `{ audioBase64, candidateDurationMs, videoFrames? }`를 정밀 분석 중계에 넘긴다. `videoFrames`는 후보당 최대 4장의 작은 JPEG 대표 화면이며 각 화면의 후보 상대 시각을 함께 보낸다. 운영 중계는 고정 prompt/schema를 조립해 `gemini-3.5-flash:generateContent`를 호출한다. 원본 파일명·전체 영상·채팅·후보 점수·사람 검토 상태는 body, URL, header, 로그에 넣지 않는다.
...
- 배포 기본값은 후보 오디오·화면 `gemini / gemini-3.5-flash`, 장시간 한국어 ASR `qwen / qwen3-asr-flash-filetrans`, 압축 방송 문맥 `qwen / qwen3.7-plus`다. `gemini-3.1-pro-preview`는 어려운 최대 3개 후보의 상위 판정 정책, `qwen3.5-omni-flash`는 후보 멀티모달 폴백, `qwen3.6-flash`는 화면 챕터링 정책, `deepseek-v4-pro`는 별도 credential이 있을 때의 문맥 폴백으로만 둔다.
- production Secret은 `GEMINI_API_KEY`와 `QWEN_API_KEY`가 필요하다. `DEEPSEEK_API_KEY`가 없으면 DeepSeek는 비활성이고 Qwen 문맥 경로에 영향을 주지 않는다. 키 원문은 readiness, 오류, 브라우저 bundle, IndexedDB, export에 기록하지 않는다.
