...
기존 `CandidatePassBRun`의 identity, 후보 순서, terminal envelope 계약은 유지하되 실행 주체를 로컬 Whisper에서 사용자 시작형 Gemini 요청으로 바꾼다. 상태 이름 `loadingModel`은 하위 호환을 위해 이번 patch에서 유지하지만 UI 의미는 `원격 분석 연결 준비`이며 모델 다운로드를 뜻하지 않는다. snapshot의 runtime은 `remote`, 모델은 `gemini-3.5-flash`, revision은 고정 manifest 값이다.

1. `START_REQUESTED` 전 guard는 현재 source binding·analysis run·후보 snapshot 외에 trim된 Gemini 키와 `externalProcessingConsent=true`를 요구한다. 키와 동의는 durable snapshot에 넣지 않고 Worker request의 실행 수명에만 둔다.
...
3. Worker는 active candidate 하나만 로컬 디코드하고 WAV로 만든 뒤 Gemini에 보낸다. 응답이 와도 local snapshot의 candidate ID·range를 다시 주입하고 client exact-key·timeline fence와 reducer의 active candidate·expected proposal revision을 모두 통과하기 전에는 화면 evidence map을 갱신하지 않는다.
...
5. 인증/키 오류, 할당량, 네트워크·서비스, 구조 오류는 run-level redacted failure로 종결해 같은 키로 남은 후보 요청을 계속 보내지 않는다. 후보 로컬 decode·무음 문제만 기존 candidate gap으로 격리하고 다음 후보를 계속한다. 자동 재시도는 없다.
...
금지 전이는 키·동의를 durable snapshot에 저장, 동의 없는 외부 요청, 원본 전체·영상·채팅 전송, model 응답 candidate ID 신뢰, reducer 수용 전 evidence 기록, API 오류 원문 표시, 429 자동 반복, Gemini 문장으로 점수·순위·경계·승인 변경이다.
