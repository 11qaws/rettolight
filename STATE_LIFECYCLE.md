...
- 문서 버전: 0.3.13
- 기준 제품 계획: PRODUCT_PLAN.md 0.3.13
...
기존 `CandidatePassBRun`의 identity, 후보 순서, terminal envelope 계약은 유지하되 실행 주체를 Whisper tiny에서 기본 Gemini 요청으로 바꾼다. 상태 이름 `loadingModel`은 하위 호환을 위해 이번 patch에서 유지하지만 UI 의미는 `Gemini 연결 준비`이며 모델 다운로드를 뜻하지 않는다. snapshot의 runtime은 `remote`, 모델은 `gemini-3.1-pro-preview`, revision은 `3.1-pro-preview-07-2026` 고정 manifest 값이다.
